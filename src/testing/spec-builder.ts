...
export class ComponentSpec<T> {
  ...
  hasInput(inputProperty: keyof T, inputName?: string) {
    this.specs.push(() => {
      it('receives a [' + inputName + '] input', () => {
        ...
        expect(inputs.hasOwnProperty(inputProperty)).toBeTruthy(
          `${this.componentClass.name} does not declare a @Input(${inputName || ''}) ${inputProperty}.`
        );
        expect(inputs[inputProperty]).toBe(
          inputName,
          `${this.componentClass.name} does not declare a @Input(${inputName || ''}) ${inputProperty}.`
        );
      });
  ...
  hasOutput(outputProperty: keyof T, ouputName?: string) {
    this.specs.push(() => {
      it('offers a (' + ouputName + ') output', () => {
        ...
        expect(inputs.hasOwnProperty(outputProperty)).toBeTruthy(
          `${this.componentClass.name} does not declare a @Output(${ouputName || ''}) ${outputProperty}.`
        );
        expect(inputs[outputProperty]).toBe(
          ouputName,
          `${this.componentClass.name} does not declare a @Output(${ouputName || ''}) ${outputProperty}.`
        );
      });
  ...
  set<K extends keyof T>(property: K, value: T[K]) {
    return this.setup(function () {
      this.testedDirective[property] = value;
    });
  }
